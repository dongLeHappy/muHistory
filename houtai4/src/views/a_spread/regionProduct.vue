	...
	<div class="rowflex">
		<div class="tree-left">
		 ...
		 <div style="padding: 5px">
	        ...
	        <Card>
	            <Row>
	                <span @click="save" style="display: inline-block;padding: 3px" v-if="$util.getAccess('adv_save')"><Button type="success" icon="plus">添加区域商品分类</Button></span>
	            </Row>
	        ...
	        <Modal v-model="editShow" :title="modalTitle" :width="600" :mask-closable=false :closable=false>
	            <Form ref="viewModel" :model="viewModel" :rules="validateRules" :labelWidth="90" inline>
	                <Row>
	                    ...
	                    <Row>
                        <FormItem prop="regionCode" label="适用地区" style="width: 700px">
	                ...
	                <Row>
                        <FormItem prop="effectTime" label="上架时间" >
                            <DatePicker type="datetime"  format="yyyy-MM-dd 00:00:00"
                            	 v-model="viewModel.effectTime" style="width: 180px" :disabled="disableEdit"></DatePicker>
                        </FormItem>
	                ...
	                <Row>
	                    <FormItem label="区域商品分类图片" v-if="disableEdit">
	                    ...
	                    <div style="margin-left: 30px" v-else>
	                        <p style="margin:5px"><b>区域商品分类图片</b></p>
    ...
    export default {
        data () {
            ...
            return {
                ...
                columns: [//字段
                	{title: '区域商品分类id', key: 'id', align:'center'},
                	{title: '分类编码', key: 'categoryNo', align:'center'},
                  
                    {title: '父分类编码', key: 'parent', align:'center'},
                    {title: '区域商品分类名称', key: 'categoryName', align:'center'},
                    ...
                    {title: '城市', key: 'regionCode', align:'center', minWidth:110
                        , render: (h, params) => {
                        let self = this;
...
//                      else if (type === 2) type = h('div', {attrs:{style:'color:blue'}}, '快洁专员');
        ...
        methods:{
            ...
            delete(index){
                let self = this;
                self.$Modal.confirm({title:'区域商品分类删除',content:'确定要删除该商品分类吗?',onOk:() => {
                    self.$axios.post(self.deleteUrl, self.myList[index].id, {headers:{'Content-Type':'application/json'}}).then((res) => {
                        self.$Notice.success({title:'区域商品分类删除', desc:'删除成功'});
                        self.queryMyList();
                    })
                }});
            },
